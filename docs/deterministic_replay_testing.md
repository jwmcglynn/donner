...
injection hook for proving that a test would catch a timing regression.
...
- **Production behavior is unchanged at defaults.** Every knob defaults to
  ...
  worker still runs free against wall-clock; the deterministic paths are
  test-only.
...
The dominant source of nondeterminism is *when a worker render becomes visible*
...
near the bottom (`maybeRequestRender`).
...
- **`HoldFramesBehind(n)`**. Installed at the poll seam
  (`AsyncRenderer::setReplayResultHoldFramesForTesting`) rather than in the
  harness, because `runFrame()` already owns `pollRenderResult`. A completed
...
- **Worker-delay injection**.
  ...
  deterministic test can demonstrate that it would catch the timing bug; the
  suite keeps no permanent "realtime must fail" assertion.
...
The GL harness cannot reuse that simplified loop because its purpose is fidelity
to the real `runFrame()` and GL texture path; this framework brings equivalent
...
- **`//donner/editor/tests:gl_rnr_replay_tests`**. The end-to-end GL replay
  ...
  focused fixture over the `{pace on/off} x {0,5,10,20,50 ms}` matrix and asserts
...
cause false flakes; the raw diagnostics remain available for failure output.
...
- Do not add a permanent test that asserts the realtime path is flaky; the
  red-then-green evidence lives in commit history or a local witness run.
