...
- [ ] **Milestone 2 — Cache and nightly infrastructure (M effort)**
  ...
  - [x] M2.8: **Drop full Coverage from PR gate; add patch-coverage signal.** Coverage currently runs on initial PR open + main push + workflow_dispatch at ~20-30 min. Remove the PR trigger entirely; keep the main push + dispatch. Add Codecov patch-coverage (or `diff-cover`) reporting on PRs — reads the main-branch coverage artifact and annotates "X% of your changed lines are covered" without rerunning coverage on the PR. Saves 20-30 min per initial PR open.
  - [x] M2.10: **Incremental per-PR coverage via bazel-diff + Codecov carryforward.** M2.8 dropped the PR coverage gate for runtime reasons, leaving PRs reliant solely on Codecov's server-side patch signal against main's stored baseline. M2.7's `determine-targets` job unlocks a cheaper alternative: a new `pr-coverage` job in `main.yml` runs `tools/coverage.sh --quiet --no-html` against the bazel-diff affected-target set, intersected with the existing coverage scope (`//donner/base/...`, `//donner/css/...`, `//donner/svg/...`, `//donner/editor/sandbox/...`; excludes `//donner/editor/gui/...` and `//donner/editor:all` for the same hermetic-sysroot/GLFW reason as `coverage.yml`). The upload uses the same `unittests` flag as the main-branch coverage job, and `codecov.yml` opts that flag into **carryforward** so files outside the partial upload reuse main's most-recent coverage instead of being reported as 0%. Fallback rules mirror M2.7 — infra changes (`MODULE.bazel`, `WORKSPACE*`, `.bazelrc`, `build_defs/**`, `.github/workflows/**`, `.github/actions/**`) or a `ci:full-test` label skip the partial run entirely and let the existing patch-coverage signal cover the PR. Target single-file PR runtime: ≤5 min (matches the affected-test job). Full `//...` coverage stays on main + `workflow_dispatch` via `coverage.yml`. Gotchas: (a) carryforward is keyed by flag name — flag must match `unittests` exactly across both jobs ([Codecov docs](https://docs.codecov.com/docs/carryforward-flags)); (b) the PR job uses a distinct disk-cache slot (`coverage-${{ runner.os }}-coverage-pr`) so it does not evict main's `coverage-${{ runner.os }}-coverage` slot (M2.1 discipline); (c) `tools/coverage.sh` requires `--config=latest_llvm`, so this job pays the hermetic-LLVM download cost on cold cache; (d) `fail_ci_if_error: false` so Codecov upload flakes don't block PRs on this new gate.

  - [ ] M2.9: **`misc-include-cleaner` per-library lint inside `bazel test //...`.** Today the check runs only in `lint.yml` as a diff-only gate via `tools/run_misc_include_cleaner_diff.sh` — that's inconsistent with always-green policy ("`bazel test //...` is the single source of truth"). Add a per-library `_include_cleaner` test emitted by `donner_cc_library`/`_test`/`_binary` whenever the parent sets `include_cleaner = "strict"`. The new rule lives in `build_defs/include_cleaner.bzl` and reuses the standard `CcInfo` plumbing to extract per-target compile flags, then runs `clang-tidy --checks=-*,misc-include-cleaner --warnings-as-errors=*` on the library's srcs+hdrs. Default is `False` so libraries opt in directory-by-directory as historical debt (issue [#559](https://github.com/jwmcglynn/donner/issues/559), ~2,875 findings) is burned down. The diff-only `lint.yml` gate stays in place as a net for non-opted-in code. Initial opt-in: `//donner/base/element:element`. **Deferred:** `include_cleaner = "allow_historical"` baseline mode (would need a checked-in baseline file or git-blame integration) — re-evaluate once enough libraries are opted in to feel the friction.

  > **Why not just turn it on globally?** The historical debt (~2,875 findings) violates always-green if enabled project-wide. The per-library opt-in lets us bring libraries online incrementally without ever landing red on `main`.
