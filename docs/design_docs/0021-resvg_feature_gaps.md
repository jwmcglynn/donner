...
| painting/image-rendering  |     2 | Both backends implement the complete typed value set, including two-stage non-integer `pixelated` scaling. The remaining skipped legacy `optimizeSpeed` cases use an allowed nearest algorithm but disagree with the vendored golden's pixel-center grid, so they remain oracle triage rather than missing value semantics. |
