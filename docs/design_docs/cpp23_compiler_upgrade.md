...
- Raise Donner's minimum supported Clang toolchain to the highest common
  platform baseline (Clang 18 on macOS 26 and Ubuntu 24.04) to unlock C++23
  vocabulary types (`std::expected`) and utility helpers
  (`std::to_underlying`, `std::string::contains`).
- Capture the ceiling imposed by system Clang 18: newer library types like
...
- Run the compiler feature probes in CI for both Bazel and CMake builds to
  confirm Clang 18 library coverage on macOS 26 and Ubuntu 24.04.
...
- [ ] Baseline compiler matrix
  - [ ] Validate current Apple Clang (macOS 26 / Clang 18) feature coverage and
        record missing C++23 headers (`<expected>`, `<flat_map>`).
  - [ ] Confirm Ubuntu 24.04 LLVM packages (Clang 18) and remaining gaps.
- [x] Toolchain upgrade path
  - [x] Add docs and scripts to install Clang 18 on macOS via Homebrew when the
        system toolchain lags (future macOS releases may ship newer by default).
  - [x] Add docs to install Clang 18 or newer from the apt.llvm.org repository
...
- [ ] Build configuration updates
  ...
  - [x] Pin CI and developer containers to Ubuntu 24.04 with Clang 18 and
        install LLVM 18 on macOS runners to keep builds on the shared
        baseline.
  - [ ] Gate third-party deps or warnings that regress with the newer standard
        library.
- [x] Compiler feature validation
  - [x] Add test coverage to assert Clang 18 provides `std::expected`,
...
- [ ] Validation
  - [ ] Run Bazel and CMake builds on macOS with Apple Clang 18 and Ubuntu
        24.04 with Clang 18.
  - [ ] Add CI jobs for the upgraded toolchain and keep a fallback job pinned to
        the highest system Clang (Apple Clang 18) to catch portability issues.
...
- `std::expected` is available starting in Clang 16; the new system baseline of
  Clang 18 on macOS 26 and Ubuntu 24.04 ships it in `<expected>`.
...
- Toolchains
  - Keep Bazel and CMake `cc_toolchain` configs aligned to the shared system
    baseline of Clang 18 on macOS 26 and Ubuntu 24.04.
- Feature gating
...
- Error-handling primitives
  - Transition parsers to `std::expected` once Clang 16+ is baseline; this
...
- Add CI coverage for Bazel and CMake builds using Clang 18 on macOS and Ubuntu
  (system toolchains or official packages).
- Keep a canary CI build on Apple Clang 18 to catch regressions when system
...
## Dependencies
- System Clang toolchain packages (Clang 18) on macOS and Ubuntu; no new runtime
  dependencies.

## Rollout Plan
- Phase 1: land toolchain setup docs and CI coverage for Clang 18.
...
- Do we need a temporary shim or backport for `std::expected` to ease the
  transition while Apple Clang 18 remains in circulation?
