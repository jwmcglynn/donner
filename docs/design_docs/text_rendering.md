...
- **Chunk splitting uses UTF-16 indices** (not raw codepoint indices) for `xList`/`yList` lookups,
  preventing ZWJ emoji sequences (🏳️‍🌈) from being split.
...
21 failing `e-tspan-*` tests, categorized by feature:

### Category 1: Text chunk splitting (e-tspan-005, 006, 031)

Mixed text and tspan content where text flows across element boundaries.

| Test | Pixels | SVG Pattern |
|------|--------|-------------|
| 005 | 4,385 | `<tspan fill="green">Text</tspan> Text` — basic mixed content |
| 006 | 2,158 | Nested `<tspan fill="green">Text <tspan font-weight="bold">Text</tspan></tspan>` |
...
### Category 4: Per-character rotate (e-tspan-016, 017, 029)

| Test | Pixels | SVG Pattern |
|------|--------|-------------|
| 016 | 2,228 | Complex nested rotate lists across 4 levels of tspan nesting |
| 017 | 1,323 | Simple `rotate="25"` on child tspan |
| 029 | 6,889 | Rotate + `display:none` tspan — hidden chars consume rotate indices |
...
### Category 6: Cross-tspan shaping (e-tspan-023, 024)

| Test | Pixels | SVG Pattern |
|------|--------|-------------|
| 023 | 4,218 | `T<tspan fill="green">e</tspan>xt` — color change mid-word |
| 024 | 10,277 | `A<tspan font-weight="bold">V</tspan>A` — weight change + text-anchor |

**Root cause**: When a tspan changes style mid-word, shaping (kerning) should still
apply across the boundary. Currently each span is shaped independently, losing
inter-span kerning. The cross-span kerning code exists but may not handle all cases
(especially weight changes that affect advance widths).
...
#### Phase A: Text chunk splitting (fixes 005, 006, 031)

Fix `TextSystem::collectSpans` to correctly handle text content that flows around child
tspan elements. The `textChunks` mechanism splits text at child boundaries during parsing;
`collectSpans` needs to interleave these chunks with child span content in DOM order.

**Tests**: e-tspan-005, e-tspan-006, e-tspan-031
...
#### Phase D: Rotate inheritance (fixes 016, 017, 029)
...
**Tests**: e-tspan-016, e-tspan-017, e-tspan-029

#### Phase E: Cross-tspan shaping (fixes 023, 024)

Shape text across tspan boundaries when only non-shaping-affecting properties change
(fill, stroke). When shaping-affecting properties change (font-weight, font-family),
shape each span independently but apply cross-span kerning.

**Tests**: e-tspan-023, e-tspan-024
