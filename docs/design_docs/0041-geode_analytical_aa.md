...
> analytic-vs-finite-sample 1px blur edge, §6) was the last remaining Geode resvg gate;
> it is now un-skipped with a verified per-backend Geode golden
> ([#625](https://github.com/jwmcglynn/donner/issues/625)). Root cause confirmed
> *not* a coverage bug: the per-pixel analytic coverage on the rotated edge is a
> correctly-centered 1px box-filter ramp and the filled area matches a 16×
...
> now **zero** remaining `disableBackend(Geode)` resvg gates for coverage reasons.
