...
element in that subtree. This matches DOM behavior: user code can remove an
...
resource resolution should prefer attached document-tree state; a stale detached
entry must not keep rendering as if the node were still in the graph.
