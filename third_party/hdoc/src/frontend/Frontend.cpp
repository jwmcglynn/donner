...
hdoc::frontend::Frontend::Frontend(int argc, char** argv, hdoc::types::Config* cfg) {
  ...
  const std::optional<std::string> argCompileCommandsJSON = program.present<std::string>("--compile-commands");

  cfg->outputFilename = program.get<std::string>("--output");
  ...
  if (argCompileCommandsJSON) {
    cfg->compileCommandsJSON = std::filesystem::path(*argCompileCommandsJSON);
  } else {
  ...
  spdlog::info("Saving JSON output to {}", cfg->outputFilename);
}
