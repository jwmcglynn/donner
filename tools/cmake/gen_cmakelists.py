...
2.  **generate_embed_resources()**
    Emits tiny CMake fragments that run the Donner
    `embed_resources.py` tool to compile fonts or other assets into
    self-contained object files.
...
def discover_embedded_packages() -> List[EmbedInfo]:
    """Return information about all embed_resources invocations."""
    infos: List[EmbedInfo] = []
    for build in Path(".").rglob("BUILD.bazel"):
        content = build.read_text()
        if "embed_resources(" not in content:
            continue
        for block in re.findall(r"embed_resources\((.*?)\)\s*", content, re.S):
            name_m = re.search(r"name\s*=\s*\"([^\"]+)\"", block)
            hdr_m = re.search(r"header_output\s*=\s*\"([^\"]+)\"", block)
            res_m = re.search(r"resources\s*=\s*{([^}]*)}", block, re.S)
            if not (name_m and hdr_m and res_m):
                continue
            name = name_m.group(1)
            header_output = hdr_m.group(1)
            resources: Dict[str, str] = {}
            for line in res_m.group(1).splitlines():
                ...
                val = val.strip().strip('"\'')
                resources[key] = val
            infos.append(EmbedInfo(str(build.parent), name, header_output, resources))
    return infos


def generate_root(embed_pkgs: List[str]) -> None:
    ...
    with path.open("w") as f:
        ...
        discovered_pkgs -= SKIPPED_PACKAGES - set(embed_pkgs)

        # Ensure helper packages from embed_resources macros are always included.
        discovered_pkgs.update(embed_pkgs)
...
#
# Step 2: embed_resources helpers
#
...
def generate_embed_resources(infos: List[EmbedInfo]) -> None:
    """Create CMakeLists.txt files to embed resources."""
    for info in infos:
        cmake = Path(info.package) / "CMakeLists.txt"
        with cmake.open("w") as f:
            ...
            for var, src in info.resources.items():
                abs_path = f"${{PROJECT_SOURCE_DIR}}/{info.package}/{src}"
                f.write(f"set({var.upper()} {abs_path})\n")
            var_prefix = _sanitize(info.name).upper()
            f.write(f"set({var_prefix}_INCLUDE_DIR ${{CMAKE_CURRENT_BINARY_DIR}}/{info.name})\n")
            f.write(f"set({var_prefix}_OUT ${{CMAKE_CURRENT_BINARY_DIR}}/{info.name}/embed_resources)\n")
            f.write(f"file(MAKE_DIRECTORY ${{{var_prefix}_OUT}})\n")
            outputs = [f"${{{var_prefix}_OUT}}/{_sanitize(Path(src).name)}.cpp" for src in info.resources.values()]
            outputs.append(f"${{{var_prefix}_OUT}}/{Path(info.header_output).name}")
            f.write("add_custom_command(\n")
            f.write("  OUTPUT " + " ".join(outputs) + "\n")
            cmd = (
                f"  COMMAND ${{Python3_EXECUTABLE}} ${{PROJECT_SOURCE_DIR}}/tools/embed_resources.py "
                f"--out ${{{var_prefix}_OUT}} --header {Path(info.header_output).name} "
            )
            cmd += " ".join([f"{var}=${{{var.upper()}}}" for var in info.resources.keys()])
            f.write(cmd + "\n")
            deps = " ".join([f"${{{var.upper()}}}" for var in info.resources.keys()])
            f.write(f"  DEPENDS {deps} ${{PROJECT_SOURCE_DIR}}/tools/embed_resources.py\n")
            f.write(f"  COMMENT \"Embedding {info.name}\"\n")
            f.write("  VERBATIM\n")
            f.write(")\n")
            target = cmake_target_name(info.package, info.name)
            src_list = " ".join(outputs[:-1])
            f.write(f"add_library({target} {src_list})\n")
            f.write(f"target_include_directories({target} PUBLIC ${{{var_prefix}_INCLUDE_DIR}})\n")
            f.write(f"set_target_properties({target} PROPERTIES CXX_STANDARD 20 CXX_STANDARD_REQUIRED YES POSITION_INDEPENDENT_CODE YES)\n")
            f.write(f"target_compile_options({target} PRIVATE -fno-exceptions)\n")
...
def main() -> None:
    ...
    infos = discover_embedded_packages()
    embed_pkgs = [info.package for info in infos]
    generate_root(embed_pkgs)
    generate_embed_resources(infos)
