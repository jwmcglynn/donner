...
def generate_svg() -> None:
    ...
    # Group libraries by CMake target name, merging sources, headers, and deps for duplicates
    libs: List[Tuple[str, str]] = []
    for p in packages:
        libs.extend((p, name) for name in query_cc_libraries(p))

    # Map each CMake target to its Bazel pkg/lib entries
    target_map: Dict[str, List[Tuple[str, str]]] = {}
    for p, lib in libs:
        tgt = cmake_target_name(p, lib)
        target_map.setdefault(tgt, []).append((p, lib))
    ...
    with out.open("w") as f:
        ...
        # collect and merge srcs, hdrs, and direct bazel deps
            srcs: List[str] = []
            hdrs: List[str] = []
            direct_deps: List[str] = []
            for p, lib in entries:
                bazel_target = f"//{p}:{lib}"
                srcs.extend(query_labels("srcs", bazel_target, relative_to="donner/svg"))
                hdrs.extend(query_labels("hdrs", bazel_target, relative_to="donner/svg"))
                direct_deps.extend(query_direct_deps_labels(bazel_target))

            write_library(f, target, srcs, hdrs)

            # link dependencies based on Bazel 'deps' attribute (merged)
            cmake_dep_targets: List[str] = []
            for dep_label in direct_deps:
                if not dep_label or any(dep_label == f"//{p}:{lib}" for p, lib in entries):
                    # skip empty or self-dependency
                    continue
