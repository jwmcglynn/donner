#!/usr/bin/env bash
# stage_wasm_assets.sh — Copies Donner WASM build output into media/ for the VS Code extension.
#
# Usage:
...
if [[ ! -d "$WASM_OUT" ]]; then
  ...
  echo "  bazel build --config=editor-wasm //donner/svg/renderer/wasm:donner_renderer_wasm"
  exit 1
fi
...
echo "WASM assets staged to $MEDIA_DIR/"
