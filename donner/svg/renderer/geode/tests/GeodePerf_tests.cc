...
void printCounters(const char* label, const geode::GeodeCounters& c) {
  std::fprintf(stderr,
               ...
               "  commandBuffers=%3" PRIu64 "  drawCalls=%4" PRIu64 "  pipelineSwitches=%3" PRIu64
               ...
               c.submits, c.commandBuffers, c.drawCalls, c.pipelineSwitches, c.sameSourceDrawPairs,
               c.bufferWrites, c.bufferWriteBytes, c.textureWriteBytes);
}
...
TEST_F(GeodePerfTest, SimpleShapes_NoDirtyPath_ZeroEncodes) {
  ...
  EXPECT_LE(c.bindgroupCreates, 1u)
      << "Steady-state bind-group creates: cached resident groups plus the readback.";
}
