...
#elif defined(DONNER_USE_FREETYPE_WITH_FONTCONFIG)
  return sk_sp<SkFontMgr>(SkFontMgr_New_FontConfig(nullptr, SkFontScanner_Make_FreeType()));
#endif
}
...
TEST(TypefaceResolverTests, PicksSystemFamilyWhenAvailable) {
  sk_sp<SkFontMgr> fontManager = CreateFontManager();
  ASSERT_TRUE(fontManager);
  if (fontManager->countFamilies() == 0) {
    GTEST_SKIP() << "No system fonts available in this SkFontMgr configuration";
  }
