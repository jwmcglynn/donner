...
to validate Donner's rendering end-to-end. The suite provides SVG inputs under `svg/`, expected PNG
goldens under `png/`, shared assets under `images/`, and fonts under `fonts/`. The Bazel target
...
The parameter-driven test scans a curated list of directories under `svg/` and instantiates a test
for every `.svg` file it finds. Test names are derived from the relative path inside `svg/` and
sanitized so that repeated file names in different directories remain unique. Every test also uses
a 500x500 canvas size by default to mirror the original suite expectations.

The harness also supports per-test configuration to allow more lenient matching or to skip tests
that exercise unsupported features. Each override is keyed by the relative path under `svg/` and
can set thresholds, mark tests as skipped, or point at alternate goldens. For example, the resvg
layout now contains `painting/stroke-dasharray/em-units.svg` (a case Donner does not yet support),
which is mapped to `ImageComparisonParams::Skip()` in
