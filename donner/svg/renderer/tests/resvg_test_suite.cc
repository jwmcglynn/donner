    ...
    Combine(ValuesIn(getTestsInCategory(
                "structure/transform-origin",
                {
                    ...
                    {"on-pattern-user-space-on-use.svg",
                     Params::Skip("transform-origin on patternTransform: paint-server transform "
                                  "gap")},
                    // transform-origin pivot now composes for textPath too (#624):
                    // resolveTextPath() applies the `transform-origin` pivot
                    // (T(-origin)*M*T(origin)) to the baseline path geometry, matching how the
                    // path element itself renders. Previously the raw `parentFromEntity` (pivot
                    // dropped) rotated the path about the origin, sampling every glyph off-screen.
                },
