...
TEST(ImageComparisonTerminalPreviewTest, RendersGridWithCaptionsAndPadding) {
  ...
  const TerminalImageView actualView{actual, 1, 2, 1};
  const TerminalImageView expectedView{expected, 1, 2, 1};
  const TerminalImageView diffView{diff, 1, 2, 1};

  TerminalImageViewerConfig viewerConfig{};
  viewerConfig.enableITermInlineImages = false;
  viewerConfig.useTrueColor = true;
  viewerConfig.scale = 1.0;
  ...
  // Note: Padding is calculated based on column widths:
  // - Left column width = max("Actual"=6, actualBlock=1) = 6
  // - Right column width = max("Expected"=8, expectedBlock=1) = 8
  // - Column padding = 2
  // So actualBlock gets 5 spaces to reach width 6, then 2 padding spaces = 7 total before
  // expectedBlock And expectedBlock gets 7 spaces to reach width 8
