  ...
  for (auto child = node.firstChild(); child; child = child->nextSibling()) {
    if (child->type() == XMLNode::Type::Data || child->type() == XMLNode::Type::CData) {
      ...
      auto parseLengthAttr = [&](std::string_view attrName, Lengthd& target) -> bool {
        ...
        ParseError err;
        err.reason =
            "Invalid " + std::string(attrName) + " value '" + std::string(maybeAttr.value()) + "'";
      ...
      if (!hasWidth || !hasHeight) {
        ...
        context.addSubparserWarning(
            std::move(err), ParserOrigin::StartOffset(*node.getNodeLocation()->start.offset));
        continue;
      }
