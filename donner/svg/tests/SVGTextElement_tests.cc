...
TEST(SVGTextElementViewportTests, SimpleLetter) {
  ...
  // The "T" glyph renders with platform-specific AA: macOS TinySkia produces
  // sharp non-AA output, Linux TinySkia produces AA edges, Skia produces
  // subpixel AA. Verify the glyph is present and roughly correct.
  const auto image = RendererTestUtils::renderToAsciiImage(doc);
