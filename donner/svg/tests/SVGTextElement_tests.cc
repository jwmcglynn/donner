...
TEST(SVGTextElementViewportTests, EmUnitsUseComputedFontSize) {
  SVGDocument doc = instantiateSubtree(
      R"-(
    <svg viewBox="0 0 64 40">
      <rect width="64" height="40" fill="black" />
      <text x="1em" y="32" font-family="Noto Sans" font-size="40px" fill="white">I</text>
    </svg>
  )-",
      kExperimentalOptions, Vector2i(64, 40));

  const AsciiImage image = RendererTestUtils::renderToAsciiImage(std::move(doc));

  int minColumn = std::numeric_limits<int>::max();
  int currentColumn = 0;
  for (char ch : image.generated) {
    if (ch == '\n') {
      currentColumn = 0;
      continue;
    }
    ...
    if (ch != '.') {
      minColumn = std::min(minColumn, currentColumn);
    }

    ++currentColumn;
  }

  ASSERT_NE(minColumn, std::numeric_limits<int>::max());
  EXPECT_GE(minColumn, 34);
  EXPECT_LE(minColumn, 46);
}
