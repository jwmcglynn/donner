...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
  ...
  // The short PR-gate corpus keeps its 1 MiB decode budget. The separate nightly target uses the
  // product's 64 MiB budget and a longer per-input deadline, including a valid 16 MiB PNG seed.
  // Both paths also exercise the loader's encoded-to-decoded amplification guard.
#ifdef DONNER_IMAGE_LOADER_FUZZ_PRODUCTION_BUDGET
  const size_t maximumDecodedImageSize = ImageLoader::kDefaultMaximumDecodedImageSize;
#else
