...
def summarize_RcString(value, internal_dict):
    ...
    sizeByte = short.GetChildMemberWithName("shiftedSizeByte").GetValueAsUnsigned()
    ...
    if is_long_string(sizeByte):
        ...
        result_data = long.GetChildMemberWithName("data").GetPointeeData(0, size)

        return read_string(result_data, size)
    else:
        result_data = short.GetChildMemberWithName("data").GetData()
...
def __lldb_init_module(debugger, internal_dict):
    ...
    debugger.HandleCommand(
        cmd_prefix + '.summarize_RcString "donner::RcString" -w donner_base'
    )
    debugger.HandleCommand("type category enable donner_base")
