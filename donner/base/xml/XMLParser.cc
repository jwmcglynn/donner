...
class XMLParserImpl : public donner::parser::ParserBase {
private:
  XMLDocument document_;
  XMLParser::Options options_;
  std::optional<donner::parser::LineOffsets> lineOffsets_;

public:
  explicit XMLParserImpl(std::string_view text, const XMLParser::Options& options)
      : donner::parser::ParserBase(text), options_(options) {}
...
private:
  donner::parser::LineOffsets lineOffsets() {
    if (!lineOffsets_) {
      lineOffsets_ = donner::parser::LineOffsets(str_);
    }
