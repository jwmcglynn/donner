    ...
    for (std::size_t nibbleIndex = 0; nibbleIndex < nibbles.size(); ++nibbleIndex) {
      ...
      if (nibble <= 9) {
        if (inExponent) {
          ...
          if (integerDigit && integerLeadingZero) {
            return std::nullopt;
          }
        ...
        if (fraction || inExponent) {
          return std::nullopt;
        }
        fraction = true;
      } else if (nibble == 0xB || nibble == 0xC) {
        if (inExponent || (!integerDigit && !fraction)) {
          return std::nullopt;
        }
        inExponent = true;
        exponentNegative = nibble == 0xC;
      } else if (nibble == 0xE) {
        if (negative || integerDigit || fraction || inExponent) {
          return std::nullopt;
        }
        negative = true;
      } else if (nibble == 0xF) {
...
bool ApplyDictOperator(uint16_t op, std::span<const Number> stack, std::size_t tableSize, bool cff2,
                       DictFields* fields) {
  if (op == 15 && !cff2) {
    return AssignOffset(stack, tableSize, &fields->charset);
  }
  if (op == 17) {
    return AssignOffset(stack, tableSize, &fields->charStrings);
  }
  if (op == 18) {
    ...
    if (!size.has_value() || !offset.has_value() || fields->privateOffset.has_value()) {
      return false;
    }
  ...
  if (op == 19) {
    return AssignOffset(stack, tableSize, &fields->subrs);
  }
  if (op == 24 && cff2) {
  ...
  if (op == 0x0C24) {
    return AssignOffset(stack, tableSize, &fields->fdArray);
  }
  if (op == 0x0C25) {
    return AssignOffset(stack, tableSize, &fields->fdSelect);
  }
  if ((op == 22 || op == 23) && cff2) {
    fields->unsupportedVariation = true;
  }
...
std::optional<DictFields> ParseDict(std::span<const uint8_t> bytes, std::size_t tableSize,
  ...
  while (cursor < bytes.size()) {
    ...
    if (first >= 28 || first == 30) {
      ...
      if (!number.has_value() || operandCount >= stackLimit) {
        return std::nullopt;
      }
...
std::optional<std::vector<uint16_t>> ParseFdSelect(std::span<const uint8_t> table,
                                                   ...
                                                   ValidationBudget* budget) {
  if (!budget->charge(glyphCount + 1)) {
    return std::nullopt;
  }
  if (!HasBytes(table, offset, 1)) {
    return std::nullopt;
  }
  std::vector<uint16_t> result(glyphCount);
  const uint8_t format = table[offset++];
  if (format == 0) {
    if (!HasBytes(table, offset, glyphCount)) {
      return std::nullopt;
    }
    for (std::size_t glyph = 0; glyph < glyphCount; ++glyph) {
      if (table[offset + glyph] >= fdCount) {
        return std::nullopt;
      }
  ...
  if (countBytes == 0 || !HasBytes(table, offset, countBytes)) {
    return std::nullopt;
  }
  ...
  const std::size_t sentinel =
      glyphBytes == 2 ? ReadBe16(table.data() + offset) : ReadBe32(table.data() + offset);
  if (sentinel != glyphCount || previousFirst >= sentinel) {
    return std::nullopt;
  }
  for (std::size_t glyph = previousFirst; glyph < sentinel; ++glyph) {
    result[glyph] = previousFd;
  }
  return result;
}
...
std::optional<std::vector<uint16_t>> ParseCff1Charset(std::span<const uint8_t> table,
  ...
  if (format == 0) {
    ...
    if (!HasBytes(table, offset, entries * 2)) {
      return std::nullopt;
    }
  ...
  while (glyph < glyphCount) {
    if (!HasBytes(table, offset, 2 + leftBytes)) {
      return std::nullopt;
    }
...
std::optional<ParsedCff> ParseCff1(std::span<const uint8_t> table, std::size_t glyphCount,
  ...
  if (!topFields.has_value() || !topFields->charStrings.has_value()) {
    return std::nullopt;
  }
  ...
  if (!topFields->fdArray.has_value() || !topFields->fdSelect.has_value()) {
    return std::nullopt;
  }
...
std::optional<ParsedCff> ParseCff2(std::span<const uint8_t> table, std::size_t glyphCount,
  ...
  if (!HasBytes(table, table[2], topLength)) {
    return std::nullopt;
  }
...
private:
  ...
  bool consumeOptionalWidth(std::size_t expected) {
    ...
    if (stackSize_ == expected + 1) {
      for (std::size_t index = 1; index < stackSize_; ++index) {
        stack_[index - 1] = stack_[index];
      }
      --stackSize_;
    }
    return true;
  ...
  bool applyStems() {
    ...
    widthSeen_ = true;
    if ((stackSize_ & 1u) != 0 || hints_ + stackSize_ / 2 > kMaximumStemHints) {
      return false;
    }
    hints_ += stackSize_ / 2;
    stackSize_ = 0;
  ...
  Flow callSubroutine(bool global, std::size_t depth) {
    ...
    if (!encoded.has_value() || depth >= kMaximumSubroutineDepth) {
      return Flow::Error;
    }
    const int64_t bias = index.count < 1240 ? 107 : (index.count < 33900 ? 1131 : 32768);
    if (*encoded > std::numeric_limits<int64_t>::max() - bias) {
      return Flow::Error;
    }
    const int64_t resolved = *encoded + bias;
    if (resolved < 0 || static_cast<uint64_t>(resolved) >= index.count) {
      return Flow::Error;
    }
    ...
    if (!bytes.has_value() || bytes->size() > kMaximumCharStringLength) {
      return Flow::Error;
    }
    active_[depth] = call;
  ...
  bool applyLines(std::size_t count) {
    if (!contourOpen_ || count == 0 || !addVertices(count)) {
      return false;
    }
    stackSize_ = 0;
    return true;
  ...
  bool applyPathOperator(uint8_t op) {
    ...
    if (op == 5) {
      return stackSize_ >= 2 && (stackSize_ & 1u) == 0 && applyLines(stackSize_ / 2);
    }
    if (op == 6 || op == 7) {
      return applyLines(stackSize_);
    }
    if (op == 8) {
      return stackSize_ >= 6 && stackSize_ % 6 == 0 && applyCurves(stackSize_ / 6);
    }
    if (op == 24) {
  ...
  bool binaryArithmetic(uint8_t op) {
    ...
    if (op == 3) {
      lhs.value = (lhs.value != 0.0 && rhs.value != 0.0) ? 1.0 : 0.0;
    }
    if (op == 4) {
      lhs.value = (lhs.value != 0.0 || rhs.value != 0.0) ? 1.0 : 0.0;
    }
    ...
    if (op == 15) {
      lhs.value = lhs.value == rhs.value ? 1.0 : 0.0;
    }
    if (op == 24) {
      lhs.value *= rhs.value;
    }
  ...
  bool unaryArithmetic(uint8_t op) {
    ...
    if (op == 5) {
      value.value = value.value == 0.0 ? 1.0 : 0.0;
    }
    if (op == 9) {
      value.value = std::abs(value.value);
    }
    if (op == 14) {
  ...
  bool transientOperator(uint8_t op) {
    if (op == 18) {
      return stackSize_ != 0 && (--stackSize_, true);
    }
    if (op == 20) {
      const auto index = popKnownInteger();
      if (!index.has_value() || *index < 0 || *index >= 32 || stackSize_ == 0) {
        return false;
      }
    ...
    if (op == 27) {
      return stackSize_ != 0 && push(stack_[stackSize_ - 1]);
    }
    if (op == 28) {
      if (stackSize_ < 2) {
        return false;
  ...
  bool conditionalOperator(uint8_t op) {
    ...
    if (op == 23) {
      return push(Number{0.0, false});
    }
    if (op != 30 || stackSize_ < 2) {
      return false;
    }
  ...
  bool applyEscaped(uint8_t op) {
    if (op == 34) {
      return clearStack(7) && applyCurves(2);
    }
    if (op == 35) {
      return clearStack(13) && applyCurves(2);
    }
    if (op == 36) {
      return clearStack(9) && applyCurves(2);
    }
    if (op == 37) {
      return clearStack(11) && applyCurves(2);
    }
    if (cff2_) {
      return false;
    }
    ...
    if (op == 5 || op == 9 || op == 14 || op == 26) {
      return unaryArithmetic(op);
    }
  ...
  Flow finishCff1() {
    if (stackSize_ == 0 || (!widthSeen_ && stackSize_ == 1)) {
      if (!consumeOptionalWidth(0) || stackSize_ != 0 || !closeContour()) {
        return Flow::Error;
      }
      return Flow::EndGlyph;
    }
    if (!consumeOptionalWidth(4) || stackSize_ != 4) {
      return Flow::Error;
    }
  ...
  Flow applyOperator(uint8_t op, std::span<const uint8_t> bytes, std::size_t* cursor,
    ...
    if (op == 19 || op == 20) {
      if (!applyStems() || hints_ == 0) {
        return Flow::Error;
      }
      const std::size_t maskBytes = (hints_ + 7) / 8;
      if (!HasBytes(bytes, *cursor, maskBytes) || !charge(maskBytes)) {
        return Flow::Error;
      }
      *cursor += maskBytes;
      return Flow::Return;
    ...
    if (op == 10 || op == 29) {
      return callSubroutine(op == 29, depth);
    }
    if (op == 11) {
      return subroutine ? Flow::Return : Flow::Error;
    }
    if (op == 14) {
      return !cff2_ ? finishCff1() : Flow::Error;
    }
    if (op == 15 || op == 16) {
      return cff2_ ? Flow::Unsupported : Flow::Error;
    }
    return applyPathOperator(op) ? Flow::Return : Flow::Error;
  }

  Flow run(std::span<const uint8_t> bytes, bool subroutine, std::size_t depth) {
    if (bytes.size() > kMaximumCharStringLength) {
      return Flow::Error;
    }
    std::size_t cursor = 0;
    while (cursor < bytes.size()) {
      ...
      if (first == 12) {
        if (cursor >= bytes.size() || !charge(1)) {
          return Flow::Error;
        }
        if (!applyEscaped(bytes[cursor++])) {
          return Flow::Error;
        }
        continue;
      }
      const Flow flow = applyOperator(first, bytes, &cursor, subroutine, depth);
      if (flow == Flow::Error || flow == Flow::Unsupported || flow == Flow::EndGlyph) {
        return flow;
      }
      if (first == 11) {
        return Flow::Return;
    ...
    if (cff2_) {
      return subroutine ? Flow::Return : Flow::EndGlyph;
    }
    return Flow::Error;
  }
...
private:
  ...
  bool addWork(std::size_t* work, std::size_t count) const {
    if (*work > kMaximumGlyphWork || count > kMaximumGlyphWork - *work) {
      return false;
    }
    *work += count;
    return true;
  ...
  bool addVertices(std::size_t* vertices, std::size_t count) const {
    if (*vertices > kMaximumVertices || count > kMaximumVertices - *vertices) {
      return false;
    }
    *vertices += count;
    return true;
  ...
  bool resolve(std::size_t glyph, std::size_t currentDepth) {
    ...
    if (raw_[glyph].components.has_value()) {
      ...
      for (const std::size_t component : *raw_[glyph].components) {
        if (component >= raw_.size() || !resolve(component, currentDepth + 1)) {
          return false;
        }
      ...
      if (depth > kMaximumSubroutineDepth) {
        return false;
      }
      if (!budget_->charge(2)) {
        return false;
