...
enum class GpuErrorType : uint8_t {
  ...
  /// the results it was to produce cannot be trusted. Distinct from \ref
  /// donner::gpu::GpuErrorType::InvalidState because a caller recovers from the two differently: an
  /// invalid state is a mistake to correct and retry, a lost device is gone and its resources with
  /// it.
  DeviceLost,
};
