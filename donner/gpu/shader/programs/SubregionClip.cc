...
ShaderResult<IrModule> BuildClipModule() {
  ErrorLatch e;
  ...
  const IrType paramsType = e(IrType::Struct(
      ...
      {IrType::Member{"invA", f32}, IrType::Member{"invB", f32}, IrType::Member{"invC", f32},
       ...
       // Ten f32 members give the struct 4-byte alignment and so a 40-byte size. The two
       // trailing words carry that to 48, which is the size a host mirror declared with 16-byte
  ...
  e.ok(AddBindings(builder, paramsType, StorageTextureFormat::Rgba32Float));
  ...
  // whole workgroups cannot store out of bounds. The bound comes from the texture being written,
  // where the shader this replaces read it from the texture being sampled. The two are the same
  // size, so the emitted code is equivalent; reading it from the destination is what keeps the
  // check tied to the storage it guards.
  const IrExpr extent =
