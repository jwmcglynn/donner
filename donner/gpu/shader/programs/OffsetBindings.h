#pragma once
/// @file
/// Bind group indices of the SVG offset filter program.
///
...
inline constexpr std::string_view kOffsetEntryPoint = "cs_main";
...
inline constexpr uint32_t kOffsetWorkgroupSize = 8;

/// Bind group indices of the offset filter program, shared by the IR builder and every host that
/// creates its bind group layout.
enum class OffsetBinding : uint32_t {
  InputTexture = 0,   //!< Sampled `texture_2d<f32>` source.
  OutputTexture = 1,  //!< `texture_storage_2d<rgba8unorm, write>` destination.
  Params = 2,         //!< Uniform buffer holding the shift in pixels.
};
