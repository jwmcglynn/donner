#pragma once
/// @file
/// Authoritative WGSL for Phong specular lighting over an alpha height map.
#include "donner/gpu/shader/wgsl/Compiler.h"
namespace donner::gpu::shader::programs {
/// Authoritative WGSL for Phong specular lighting over an alpha height map.
inline constexpr wgsl::SourceText kSpecularLightingSource{R"wgsl(struct LightingParams {
  surfaceScale: f32,
...
fn cs_main(@builtin(global_invocation_id) gid: vec3<u32>) {
  ...
  let halfway = safeNormalize((lightDirection + vec3<f32>(0f, 0f, 1f)));
  let intensity = ((params.lightingConstant * svgPow(max(dot(normal, halfway), 0f), params.specularExponent)) * spot);
  ...
  textureStore(outputTexture, coord, vec4<f32>(red, green, blue, max(red, max(green, blue))));
}
)wgsl"};
}  // namespace donner::gpu::shader::programs
