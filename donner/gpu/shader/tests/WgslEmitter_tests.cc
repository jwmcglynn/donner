/// @file
/// WGSL emitter tests: construct coverage, determinism, and fail-closed rejection of
...
TEST(WgslEmitterTests, CoverageModuleEmitsBindingsAndControlFlow) {
  const auto wgsl = EmitWgsl(BuildEmitterCoverageModule());
  ASSERT_THAT(wgsl, HasShaderResult());
  EXPECT_THAT(
      wgsl.result(),
      testing::AllOf(HasSubstr("@group(0) @binding(0) var<uniform> params"),
                     HasSubstr("@group(0) @binding(1) var<storage, read> values"),
                     HasSubstr("@group(0) @binding(2) var tex"),
                     HasSubstr("@group(0) @binding(3) var smp"), HasSubstr("fn sum_values("),
                     HasSubstr("@vertex"), HasSubstr("@fragment"), HasSubstr("for ("),
                     HasSubstr("continue;"), HasSubstr("discard;"), HasSubstr("textureSample("),
                     HasSubstr("textureLoad(")));
}
