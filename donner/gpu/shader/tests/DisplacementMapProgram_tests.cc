/// @file
/// DisplacementMap frozen-artifact metadata and projection tests.
...
TEST(DisplacementMapProgramTests, FreezesAllProjectionsAndDerivedMetadata) {
  const CompiledShaderView& shader = tests::DisplacementMapAllProjections();
  ...
  ASSERT_NE(shader.resource("mapTexture"), nullptr);
  EXPECT_EQ(shader.resource("mapTexture")->group, 0u);
  EXPECT_EQ(shader.resource("mapTexture")->binding, 1u);
  ...
  EXPECT_EQ(shader.resource("params")->minSizeBytes, sizeof(programs::DisplacementMapParams));
  EXPECT_NE(shader.wgsl.find("struct DisplacementParams"), std::string_view::npos);
  EXPECT_EQ(shader.spirv.front(), 0x07230203u);
}

TEST(DisplacementMapProgramTests, AdapterArtifactRetainsOnlyWgsl) {
  const CompiledShaderView& adapter = programs::DisplacementMapShader();
  EXPECT_EQ(adapter.wgsl, tests::DisplacementMapAllProjections().wgsl);
  EXPECT_TRUE(adapter.msl.empty());
  EXPECT_TRUE(adapter.spirv.empty());
  EXPECT_EQ(adapter.resources.size(), tests::DisplacementMapAllProjections().resources.size());
  EXPECT_EQ(adapter.entryPoints.size(), tests::DisplacementMapAllProjections().entryPoints.size());
}

TEST(DisplacementMapProgramTests, MutationControlReflectsChangedInterface) {
  const CompiledShaderView& shader = tests::DisplacementMapMutatedAllProjections();
  ASSERT_THAT(shader.entryPoints, testing::SizeIs(1));
  ...
  ASSERT_NE(shader.resource("mapTexture"), nullptr);
  EXPECT_EQ(shader.resource("mapTexture")->binding, 5u);
  ASSERT_NE(shader.resource("outputTexture"), nullptr);
