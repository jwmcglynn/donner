...
constexpr auto kArtifact = wgsl::Compile<programs::kFloodSource, wgsl::Projection::All>();
constexpr CompiledShaderView kView = kArtifact.view();
static_assert(programs::ValidateFloodArtifact<kView>());
constexpr auto kMutatedSource = ReplaceExact(
    ReplaceExact(ReplaceExact(ReplaceExact(programs::kFloodSource, "@binding(0)", "@binding(6)"),
                              "@binding(1)", "@binding(5)"),
...
static_assert(programs::ValidateFloodArtifact<kMutatedView>());
}  // namespace
const CompiledShaderView& FloodAllProjections() {
  return kView;
}
const CompiledShaderView& FloodMutatedAllProjections() {
  return kMutatedView;
}
}  // namespace donner::gpu::shader::tests
