...
constexpr auto kArtifact = wgsl::Compile<programs::kMergeSource, wgsl::Projection::All>();
constexpr CompiledShaderView kView = kArtifact.view();
static_assert(programs::ValidateMergeArtifact<kView>());
constexpr auto kMutatedSource =
    ReplaceExact(ReplaceExact(ReplaceExact(ReplaceExact(ReplaceExact(programs::kMergeSource,
                                                                     "@binding(0)", "@binding(6)"),
                                                        "@binding(1)", "@binding(5)"),
                                           "@binding(2)", "@binding(4)"),
...
static_assert(programs::ValidateMergeArtifact<kMutatedView>());
}  // namespace
