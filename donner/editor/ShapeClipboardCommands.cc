...
PreparePasteResult preparePaste(const svg::SVGDocument& document,
  ...
  // malformed clipboard never mutates the destination document.
  {
    ...
    // Validate with the same parse options the editor applies on paste, so
    // validation and the actual insert agree on what is representable.
    auto parsed = svg::parser::SVGParser::ParseSVG(wrapped, sink, EditorParseOptions());
    if (parsed.hasError()) {
