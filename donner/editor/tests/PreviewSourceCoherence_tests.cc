...
TEST(PreviewSourceCoherence, MatchesAfterEachCommittedOperation) {
  ...
  // (d2) Convert Text to Outlines on the original fixture `<text>`. This is the
  // structural-reparse commit path (`ConvertTextToOutlinesCommand`), identical
  // in mechanism to a shape-clipboard Cut/Paste reparse — it swaps the whole
  // document to the outlined source. Verifies the preview still equals a
  // save->reload after a structural document swap.
  {
    const svg::SVGElement label = Require(async, "#label");
    const ConvertTextToOutlinesResult result = convertTextToOutlines(async.document(), label);
  ...
  // driven here. Its commit mechanism is the SAME structural-reparse path
  // (`AsyncSVGDocument::applyOne` handles CutShapes/PasteShapes/
  // ConvertTextToOutlines identically — reparse `bytes` via
  // `setDocumentMaybeStructural`), which IS covered by the
  // `convert_text_to_outlines` step above. Driving the clipboard helper itself
  // requires selection-id plumbing that belongs to `shape_clipboard_tests`; the
  // coherence-relevant apply path is already exercised.
}
