...
void EditorShell::updateWindowTitle() {
  ...
  // On platforms with native title-bar chrome (macOS), the OS shows document
  // state, so the title text stays the exact product name.
  const bool nativeChrome = NativeWindowChromeAvailable();
  std::string title = ComposeWindowTitle(chromeState, /*showDocumentStateInText=*/!nativeChrome);
