...
static bool isTwoCharacterNewline(std::string_view str) {
...
static std::tuple<char32_t, int> consumeEscapedCodepoint(std::string_view remaining) {
  if (remaining.empty()) {
    ...
    if (remaining[0] != '\0') {
      const auto [codepoint, bytesConsumed] = Utf8::NextCodepoint(remaining);
...
bool Tokenizer::isEOF() const {
  return remaining_.empty() && !nextToken_;
}

size_t Tokenizer::currentOffset() const {
  return remaining_.data() - str_.data();
}

std::optional<Token::ErrorToken> Tokenizer::consumeComments() {
...
Token Tokenizer::consumeQuotedString() {
...
Token Tokenizer::consumeNumericToken() {
  ...
  if (isIdentifierStart(remainingAfterNumber)) {
    ...
    return token<Token::Number>(number.consumedChars, number.number, std::move(numberString), type);
  }
}
...
Token Tokenizer::consumeIdentLikeToken() {
  ...
  if (name.equalsLowercase("url") && hasParen) {
    ...
    while (i + 1 < remainingSize && isWhitespace(afterName[i]) && isWhitespace(afterName[i + 1])) {
      ++i;
    }
...
Token Tokenizer::consumeUrlToken(const std::string_view afterUrl, size_t charsConsumedBefore) {
...
Token Tokenizer::consumeRemnantsOfBadUrl(const std::string_view badUrl,
                                         size_t charsConsumedBefore) {
  size_t i = 0;
