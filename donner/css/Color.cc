...
constexpr auto kColorEntries = std::to_array<std::pair<std::string_view, Color>>({
...
constexpr auto kColorsResult = makeCompileTimeMap(kColorEntries);
static_assert(kColorsResult.status == CompileTimeMapStatus::kOk);
constexpr auto& kColors = kColorsResult.map;
