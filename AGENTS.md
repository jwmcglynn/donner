...
2. **Run `bazel test //...`** before opening the PR. This is the single source of truth for local validation — it covers:
   ...
   - The per-library clang-format lint (`*_clang_format` py_tests auto-emitted by `donner_cc_library`/`_test`/`_binary`). Catches format escapes against the project `.clang-format` at test time, so `clang-format --dry-run -Werror` doesn't only run in CI.
   ...
   - **`clang-format -i` on every modified C/C++ file** before committing — `git clang-format` covers staged changes. The per-target `*_clang_format` py_tests above will catch any escapes if you forget.
