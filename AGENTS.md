...
- **Flaky tests are fixed at the root cause, as a high priority.** A test that fails twice on unrelated changes gets a root-cause investigation, not a third rerun. Never mask a flake with a longer timeout, a retry, or a weakened assertion: find the wrong observable (a wait on state that does not prove what the assertion measures) or the product race, and fix that.
