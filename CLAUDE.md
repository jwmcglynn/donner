...
- **Always prefix AI-generated GitHub *comments* with 🤖.** This applies to PR comments, review comments, and issue comments posted by any AI agent (Claude, Codex, Copilot, etc.). It distinguishes human comments from AI comments, since all AI activity goes through `jwmcglynn`'s GitHub account.
- **The 🤖 emoji is reserved for comments.** PR, issue, and task *descriptions* (the body created with the PR/issue/task) must **not** carry the 🤖 emoji and must **not** state that they were written/generated by an LLM or "opened on someone's behalf." Write the description as a normal, attribution-free summary of the change. (Authorship is already implicit from the account; the 🤖 marker is only for distinguishing follow-up comments.)
...
- **Editor visual bugs**: follow [`docs/editor_visual_debugging.md`](docs/editor_visual_debugging.md)
  ...
  `bazel run --config=geode //donner/editor/tests:editor_rnr_gl_replay -- ...`; use
  `--crop document-canvas` for canvas bugs and `--crop full` for ImGui chrome, sidebars, and layer
  thumbnails.
- **Editor QA workflow is MCP-first, then red→green, then MCP-verified.** For every editor QA bug,
  first expand the editor-control MCP so it can reproduce the user-visible interaction or visual
  failure. If the MCP cannot yet drive the needed gesture, viewport state, render handoff, capture,
  or assertion, add the smallest MCP feature required and validate that repro path before touching
  the product fix. Next, build the narrowest useful unit or integration regression test and prove it
  fails on the broken code. Then fix the underlying editor issue until that test turns green.
  Finally, rerun the MCP repro and record the MCP evidence that the actual editor-facing behavior is
  fixed. A lower-level green test without MCP re-verification is insufficient for visual or
  interaction-driven QA bugs.
...
- **All new or edited tests must be ToTT-compliant before publishing.** Use matchers for arrays, byte buffers, record sequences, structs, optionals, variants, pixels, geometry, and other multi-field contracts; scalar `EXPECT_EQ` is acceptable only when the failure already prints the complete expected and actual value. Avoid bare boolean assertions (`EXPECT_TRUE(...)`, `EXPECT_FALSE(...)`, `ASSERT_TRUE(...)`) unless the condition itself is the named contract; prefer `EXPECT_THAT(value, matcher)` or add a matcher/helper that explains the mismatch. Add `PrintTo`/`operator<<` for opaque failure output, and promote repeated assertion shapes into named helpers.
...
- **Local developer test cases are capped at 10 seconds each.** Treat any gtest case that takes more than 10s on a local dev box as a bug in the test design or production testability. Split the case, sample or compact expensive replays, replace wall-clock waits with deterministic hooks, or refactor the code under test until the local case fits under the cap. If a genuinely long run is still needed, keep it out of the normal local/PR gate behind an explicit `manual` or `perf` target with a short justification; do not publish coverage-drive tests that normalize >10s local cases.
...
- **Stating a sample count is fine; blaming it is not.** "Geode renders directly at one sample per pixel" is a true config fact. "The text suite fails because its coverage method differs" is a banned root-cause claim. Describe the *symptom* (e.g. "~700px/glyph positional drift vs tiny-skia, root cause not yet identified") and open the investigation — do not close it with "AA".
...
- **Curves must never be visible as line segments.** A rendered curve showing polygon edges at any zoom is a critical rendering defect, the same severity class as drawing vector graphics with ImGui. Any pipeline that flattens curves on the CPU (stroke expansion, clipping, hit regions that feed rendering) must derive its chord tolerance from the device transform the geometry is actually drawn with, holding the error below a small constant in DEVICE pixels; a fixed tolerance in path-local or document space is banned because magnification makes the polygon visible. Callers of flattening APIs pass an explicitly derived tolerance and state its space in a comment; do not rely on a library default.
