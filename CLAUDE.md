...
- **The one exception is user-generated *text* operations** — the source/text-editor pane where the user is literally typing and editing characters. There the user IS editing the text, so a reparse-from-text path is correct. This exception covers freeform text editing only; it does NOT license structural operations to take a source-surgery shortcut.
