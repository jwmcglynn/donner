...
public:
   ...
   *
   * Each parameter is a `<number>` type, as parsed by NumberParser. `<width>` and `<height>` must
   * be positive, but the caller must ensure that they are non-zero; a value of zero should disable
