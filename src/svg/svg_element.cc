...
static std::optional<SVGElement>
querySelectorSearch(const css::Selector &selector, SVGElement element) {
...
Entity SVGElement::entity() const { return handle_.entity(); }
...
void SVGElement::setClassName(std::string_view name) {
  ...
  handle_.get_or_emplace<components::AttributesComponent>().setAttribute(
      XMLAttribute("class"), RcString(name));
}
...
void SVGElement::setStyle(std::string_view style) {
  ...
  handle_.get_or_emplace<components::AttributesComponent>().setAttribute(
      XMLAttribute("style"), RcString(style));
}

ParseResult<bool>
SVGElement::trySetPresentationAttribute(std::string_view name,
  ...
  // "transform".
  if (((type() == ElementType::LinearGradient ||
        type() == ElementType::RadialGradient) &&
  ...
  ParseResult<bool> trySetResult =
      handle_.get_or_emplace<components::StyleComponent>()
          .trySetPresentationAttribute(handle_, actualName, value);

  if (trySetResult.hasResult() && trySetResult.result()) {
    // Set succeeded, so store the attribute value.
    handle_.get_or_emplace<components::AttributesComponent>().setAttribute(
        XMLAttribute(RcString(name)), RcString(value));
    return true;
  }
...
bool SVGElement::hasAttribute(const XMLAttributeRef &name) const {
  return handle_.get_or_emplace<components::AttributesComponent>().hasAttribute(
      name);
}

std::optional<RcString>
SVGElement::getAttribute(const XMLAttributeRef &name) const {
  return handle_.get_or_emplace<components::AttributesComponent>().getAttribute(
      name);
}

void SVGElement::setAttribute(const XMLAttribute &name,
                              std::string_view value) {
  // First check some special cases which will never be presentation attributes.
  if (name == XMLAttributeRef("id")) {
    return setId(value);
  } else if (name == XMLAttributeRef("class")) {
    return setClassName(value);
  } else if (name == XMLAttributeRef("style")) {
  ...
  if (name.namespacePrefix.empty()) {
    const ParseResult<bool> trySetResult =
        trySetPresentationAttribute(name.name, value);
    if (trySetResult.hasResult() && trySetResult.result()) {
  ...
  return handle_.get_or_emplace<components::AttributesComponent>().setAttribute(
      name, RcString(value));
}

void SVGElement::removeAttribute(const XMLAttributeRef &name) {
  // First check some special cases which will never be presentation attributes.
  if (name == XMLAttributeRef("id")) {
    setId("");
  } else if (name == XMLAttributeRef("class")) {
    ...
    if (name.namespacePrefix.empty()) {
      [[maybe_unused]] ParseResult<bool> trySetResult =
          handle_.get_or_emplace<components::StyleComponent>()
              .trySetPresentationAttribute(handle_, name.name, "initial");
    }
    // Ignore return result, since it's fine if the attribute doesn't exist.
  }

  // Remove any storage for this attribute.
  handle_.get_or_emplace<components::AttributesComponent>().removeAttribute(
      name);
}
...
std::optional<SVGElement> SVGElement::parentElement() const {
  ...
  return tree.parent() != entt::null
             ? std::make_optional(SVGElement(toHandle(tree.parent())))
             : std::nullopt;
}
...
std::optional<SVGElement> SVGElement::lastChild() const {
  ...
  return tree.lastChild() != entt::null
             ? std::make_optional(SVGElement(toHandle(tree.lastChild())))
             : std::nullopt;
}
...
SVGElement SVGElement::insertBefore(SVGElement newNode,
...
SVGElement SVGElement::appendChild(SVGElement child) {
  handle_.get<components::TreeComponent>().appendChild(registry(),
                                                       child.handle_.entity());
  return child;
}

SVGElement SVGElement::replaceChild(SVGElement newChild, SVGElement oldChild) {
  handle_.get<components::TreeComponent>().replaceChild(
      registry(), newChild.handle_.entity(), oldChild.handle_.entity());
  return newChild;
}

SVGElement SVGElement::removeChild(SVGElement child) {
  handle_.get<components::TreeComponent>().removeChild(registry(),
                                                       child.entity());
  return child;
}
...
std::optional<SVGElement> SVGElement::querySelector(std::string_view str) {
  const ParseResult<css::Selector> selectorResult =
      css::SelectorParser::Parse(str);
...
EntityHandle SVGElement::CreateEntity(Registry &registry, RcString typeString,
                                      ElementType type) {
  Entity entity = registry.create();
  registry.emplace<components::TreeComponent>(entity, type,
                                              std::move(typeString));
