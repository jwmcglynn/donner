...
public:
  ...
  ParseResult<std::vector<DeclarationOrAtRule>> parse() {
    ...
    while (!tokenizer_.isEOF()) {
      ...
      if (token.is<Token::Whitespace>() || token.is<Token::Semicolon>()) {
        ...
        auto atRuleResult = consumeAtRule(std::move(token.get<Token::AtKeyword>()));
        ...
        auto maybeDeclaration =
            consumeDeclaration(SubTokenizer(declarationInput, eofOffset), std::move(ident));
        ...
        auto componentValueResult = consumeComponentValue(tokenizer_, std::move(token));
  ...
  ParseResult<AtRule> consumeAtRule(Token::AtKeyword&& atKeyword) {
    ...
    while (!tokenizer_.isEOF()) {
      ...
      if (token.is<Token::Semicolon>()) {
        ...
        // at-rule.
        auto blockResult = consumeSimpleBlock(tokenizer_, std::move(token));
        ...
        auto componentResult = consumeComponentValue(tokenizer_, std::move(token));
  ...
  static ParseResult<SimpleBlock> consumeSimpleBlock(T& tokenizer, Token&& firstToken) {
    ...
    while (!tokenizer.isEOF()) {
      ...
      if (token.tokenIndex() == endingTokenIndex) {
        ...
        // anything else: Reconsume the current input token. Consume a component value and append it
        // to the value of the block.
        auto componentResult = consumeComponentValue(tokenizer, std::move(token));
        if (componentResult.hasError()) {
          return std::move(componentResult.error());
        }
  ...
  static ParseResult<ComponentValue> consumeComponentValue(T& tokenizer, Token&& token) {
    if (token.is<Token::CurlyBracket>() || token.is<Token::SquareBracket>() ||
      ...
      // and return it.
      return consumeSimpleBlock(tokenizer, std::move(token))
          .template map<ComponentValue>(
              [](SimpleBlock&& block) { return ComponentValue(std::move(block)); });
    } else if (token.is<Token::Function>()) {
      // Otherwise, if the current input token is a <function-token>, consume a function and return
      // it.
      return consumeFunction(tokenizer, std::move(token.get<Token::Function>()))
          .template map<ComponentValue>(
  ...
  static ParseResult<Function> consumeFunction(T& tokenizer, Token::Function&& functionToken) {
    ...
    while (!tokenizer.isEOF()) {
      ...
      if (token.is<Token::CloseParenthesis>()) {
        ...
        auto componentValueResult = consumeComponentValue(tokenizer, std::move(token));
        if (componentValueResult.hasError()) {
          return std::move(componentValueResult.error());
        }
