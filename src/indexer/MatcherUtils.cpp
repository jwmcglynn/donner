...
void fillOutSymbol(hdoc::types::Symbol& s, const clang::NamedDecl* d, const std::filesystem::path& rootDir) {
  ...
  s.file = hdoc::utils::pathToRelative(*absPath, rootDir.string());
}
...
bool isInIgnoreList(const clang::Decl*              d,
  ...
  const std::string rootDirStr = rootDir.string();

  const std::string relPath = hdoc::utils::pathToRelative(*absPath, rootDirStr);
