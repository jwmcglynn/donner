...
| `{name}_soak`       | cc_test   | Exploratory mutation. `-max_total_time` is 2 s by default and 30 s under `--config=fuzz-soak` (the nightly Fuzz lane). Corpus regression coverage lives in `{name}`, not here.                                                                                                                                           |
