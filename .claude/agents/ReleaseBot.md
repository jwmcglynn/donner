...
Donner SVG Editor & Engine** (`MODULE.bazel` version `0.8.0-pre`; the showcase foundation is
implemented and release QA remains). The first
...
- `docs/design_docs/0047-v0_8_showcase.md` — the current v0.8 showcase implementation summary.
  **Status: Implemented**; release QA remains in progress.
- `docs/design_docs/0059-v0_8_showcase_retrospective.md` — implementation retrospective and link
  to the preserved original plan.
...
2. **Doxygen warning-free** — run `tools/doxygen.sh` with output captured to a log, verify its exit
   status, then verify `grep -Ei '(^|: )warning:' <log>` produces no matches.
...
6. **Showcase demo gate** (v0.8+) — the automated fixture must generate on demand from the canonical
   splash, parse, render, and appear in the built-in sample catalog; gated by
   ...
   `//donner/editor/tools:generate_showcase_asset_cli_tests`. The release-facing composition must
   then be styled live and screen recorded under operator direction using
   `docs/release_checklists/v0_8_showcase_checklist.md`.
